...
const Icons = {
  ...
  fileCode: (
    <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z" /><polyline points="14 2 14 8 20 8" /><polyline points="10 13 8 15.5 10 18" /><polyline points="14 13 16 15.5 14 18" /></svg>
  ),
  folder: (
  ...
  folderOpen: (
    <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M5 19a2 2 0 0 1-2-2V7a2 2 0 0 1 2-2h4l2 2h4a2 2 0 0 1 2 2v1" /><path d="M4.5 13h15.2a2 2 0 0 1 1.94 2.5l-1.2 5a2 2 0 0 1-1.94 1.5H4a2 2 0 0 1-2-2v-7" /></svg>
  ),
  gitBranch: (
  ...
  circleError: (
    <svg width="12" height="12" viewBox="0 0 24 24" fill="#ef4444" stroke="none"><circle cx="12" cy="12" r="10" /><line x1="8" y1="8" x2="16" y2="16" stroke="#fff" strokeWidth="2.5" strokeLinecap="round" /><line x1="16" y1="8" x2="8" y2="16" stroke="#fff" strokeWidth="2.5" strokeLinecap="round" /></svg>
  ),
  circleWarn: (
    <svg width="12" height="12" viewBox="0 0 24 24" fill="#eab308" stroke="none"><circle cx="12" cy="12" r="10" /><line x1="12" y1="8" x2="12" y2="13" stroke="#fff" strokeWidth="2.5" strokeLinecap="round" /><circle cx="12" cy="16.5" r="1.2" fill="#fff" /></svg>
  ),
  circleInfo: (
    <svg width="12" height="12" viewBox="0 0 24 24" fill="#3b82f6" stroke="none"><circle cx="12" cy="12" r="10" /><line x1="12" y1="11" x2="12" y2="17" stroke="#fff" strokeWidth="2.5" strokeLinecap="round" /><circle cx="12" cy="7.5" r="1.2" fill="#fff" /></svg>
  ),
  checkCircle: (
...
function buildFileTree(files) {
  ...
  for (const path of Object.keys(files).sort()) {
    ...
    for (let i = 0; i < parts.length; i++) {
      const part = parts[i];
      if (i === parts.length - 1) {
        node[part] = { __file: true, __path: path, __lang: files[path].language };
      } else {
...
function LiveEditor() {
  ...
  const loadRepo = useCallback(async () => {
    if (!repoUrl.trim()) return alert("Please enter a repository URL or local path");
    setRepoLoading(true);
    setRepoFiles(null);
    ...
    try {
      ...
      const res = await fetch("http://127.0.0.1:8000/repo-files", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const openRepoFile = useCallback((path, filesOverride) => {
      ...
      {summary && (
        <div className="analysis-summary">
          <div className="summary-stats">
            <div className={`summary-quality quality-${summary.overall_quality || "unknown"}`}>
              {(summary.overall_quality || "analyzed").toUpperCase()}
            </div>
            <div className="summary-counts">
              <span className="stat-error">{summary.by_severity?.error || 0} errors</span>
              <span className="stat-warning">{summary.by_severity?.warning || 0} warnings</span>
              <span className="stat-info">{summary.by_severity?.info || 0} info</span>
            </div>
            <span className="summary-total">{summary.total_violations || 0} total</span>
          </div>
        ...
        <div className="editor-split">
          ...
          <div className="editor-pane">
            ...
            <div className="monaco-wrapper">
              <Suspense fallback={<div className="editor-loading"><span className="spinner" /> Loading editor…</div>}>
                <MonacoEditor
                  height="100%"
                  language={LANGUAGES.find((l) => l.key === language)?.monacoId || "plaintext"}
                  value={code}
          ...
          <div className="violations-pane">
            <div className="pane-header">
              <span className="pane-title">
                {Icons.alertTriangle}
                <span>{violations ? `${violations.length} Violations` : "Violations"}</span>
              </span>
            </div>
            <div className="violations-scroll">
              {!violations && !analyzing && (
                <div className="empty-state">
                  ...
                  <p>Click <strong>Analyze</strong> or enable <strong>Auto-analyze</strong> to find violations.</p>
                  <p className="empty-hint">Supports Python, C++, Java, JS/TS, Go, Rust</p>
                </div>
              )}
              {analyzing && (
              ...
              {violations?.map((v, i) => (
                <div key={i} className={`violation-item ${expandedViolation[i] ? "expanded" : ""}`}>
                  <div
                    className="violation-item-header"
                    onClick={() => setExpandedViolation((prev) => ({ ...prev, [i]: !prev[i] }))}
                  >
                    <span className="v-icon">{getSeverityIcon(v.severity)}</span>
                    <span className={`v-badge ${getSeverityClass(v.severity)}`}>{v.rule_code}</span>
                    <span className="v-line">L{v.line}</span>
                    <span className="v-msg">{v.message}</span>
                    <span className="expand-icon">
                      {expandedViolation[i] ? Icons.chevronDown : Icons.chevronRight}
                    </span>
                  </div>
                  {expandedViolation[i] && (
        ...
        <div className="fix-diff-section">
          <div className="fix-diff-header">
            ...
            <div className="fix-actions">
              ...
              <button className="dismiss-btn" onClick={() => setShowDiff(false)}>
                {Icons.x} Dismiss
              </button>
            </div>
          </div>
          ...
          <div className="diff-wrapper">
            <Suspense fallback={<div className="diff-loading"><span className="spinner" /> Loading diff…</div>}>
              <ReactDiffViewer
                oldValue={code}
