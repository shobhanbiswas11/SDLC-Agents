...
export default function DocuGenius() {
  ...
  const handleChatSubmit = async (e: any) => {
    ...
    try {
      const payload = mode === "github" ? { repo, access_token: token, message } : { local_path: localPath, repo: "", access_token: "", message };
      const res = await fetch("http://localhost:8000/chat", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) });
      if (!res.ok) throw new Error(await res.text());
      const data = await res.json();
      setChatMessages(prev => [...prev, { role: "ai", text: data.answer, contextFiles: data.context_files }]);
    } catch (err: any) { setChatMessages(prev => [...prev, { role: "ai", text: `Error: ${err.message}` }]); } finally { setChatLoading(false); }
  };
  ...
  const handleOpsSubmit = async (e: any) => {
    ...
    try {
      if (!currentWorkflowId) {
        const workspacePath = mode === "local" ? localPath : repo;
        const res = await fetch("http://localhost:8001/api/workflows", {
      ...
      const preStatusRes = await fetch(`http://localhost:8001/api/workflows/${currentWorkflowId}/status`);
      const preStatusData = await preStatusRes.json();
      const initialCount = preStatusData.status?.message_count || 0;

      // 2. Send the message
      const resMsg = await fetch(`http://localhost:8001/api/workflows/${currentWorkflowId}/messages`, {
        method: "POST", headers: { "Content-Type": "application/json" },
      ...
      const poll = setInterval(async () => {
        try {
          const statusRes = await fetch(`http://localhost:8001/api/workflows/${currentWorkflowId}/status`);
      ...
      setOpsMessages(prev => [...prev, { role: "ai", text: `Error: ${err.message}` }]);
      setOpsLoading(false);
    }
  };
  ...
  const terminateOpsWorkflow = async () => {
    ...
    try {
      await fetch(`http://localhost:8001/api/workflows/${opsWorkflowId}`, { method: "DELETE" });
