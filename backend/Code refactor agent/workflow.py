...
class AgentWorkflow:
    ...
    def __init__(self):
        ...
        # Navigation & file-creation events (polled by the frontend)
        self._navigated_file: str = ""   # last file the agent navigated to
        self._created_files: list[str] = []  # new files created this session
    ...
    def get_status(self) -> dict:
        return {
            ...
            # Navigation events (frontend reacts to these)
            "navigated_file": self._navigated_file,
            "created_files": list(self._created_files),
        }
        ...
        for _ in range(max_steps):
            ...
            for tool_call in tool_calls:
                ...
                if tool_name == "ask_user":
                    ...
                    # ── React to navigation / file-creation events ──
                    if tool_name == "navigate_to_file" and tool_result.get("status") == "success":
                        nav_path = tool_result.get("resolved_path") or tool_args.get("file_path", "")
                        if nav_path:
                            self._navigated_file = nav_path
 
                    elif tool_name == "read_file" and tool_result.get("status") == "success":
                        # Also treat a read_file as navigation so the viewer auto-opens
                        rp = tool_result.get("resolved_path") or tool_args.get("file_path", "")
                        if rp:
                            self._navigated_file = rp
                        ...
                        if tool_result.get("created"):
                            ...
                            if fp and fp not in self._created_files:
                                self._created_files.append(fp)
                        # Also navigate to the newly written file
                        nav = tool_result.get("file_path") or tool_args.get("file_path", "")
                        if nav:
                            self._navigated_file = nav
