...
def get_embeddings(
    ...
    """
    Embed a list of text strings using the Azure OpenAI Embeddings API.
    Batches requests to stay within API limits.

    Returns a list of embedding vectors (same order as input).
    Returns None in the slot if a particular embedding failed.
    """
    ...
    for start in range(0, len(texts), batch_size):
        ...
        try:
            ...
            # Leave those slots as None (will be filtered out)
...
def semantic_rerank(
    ...
    Args:
        ...
        client:     AzureOpenAI client (already authenticated).
    ...
    new_embeddings = get_embeddings(texts_to_embed, client, model)
