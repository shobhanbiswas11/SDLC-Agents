...
def normalize_github_repo(repo: str) -> Tuple[str, str]:
    """
    Normalize GitHub repo input to (owner, repo_name).
...
def _fetch_one(owner: str, repo: str, path: str, token: str, branch: str) -> Tuple[str, Optional[str]]:
    """Fetch a single file. Returns (path, content). On error returns (path, None)."""
    ...
    try:
        # Public repos are more reliable through raw.githubusercontent.com.
        # Private repos can still use the GitHub contents API fallback below.
    ...
    try:
        ...
        print(f"[fetch] Failed to download {path}: {exc}")
        return path, None
...
def fetch_full_repo(
    ...
    """
    owner, name = normalize_github_repo(repo)
    
    cache_key = f"github:{owner}/{name}:{branch}"
    now = time.time()
